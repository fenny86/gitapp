...
//	}
	...
	public Account select(Integer id) {
		String sql="select * from account where id=?  order by id ASC";
	...
	public int add(Account account) {
		String sql="insert into account (id,account,password,img) values (?,?,?,?)";
		try {
			int result= queryRunner.update(sql,account.getId(),account.getAccount(),account.getPassword(),account.getImage());
			return result;
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
